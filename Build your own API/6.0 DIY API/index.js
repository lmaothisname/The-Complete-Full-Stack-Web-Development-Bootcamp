...
app.get("/jokes/:id", (req,res) => {
  ...
  const foundJoke = jokes.find((joke) => joke.id === searchId);
  res.json(foundJoke);
});
...
app.delete("/jokes/:id",(req,res) => {
  ...
  if (searchIndex > -1){
    ...
    res.status(400).json({error: `Joke with id:${id} not found. No joke deleted!`});
  }
});
//8. DELETE All jokes
app.delete("/all", (req,res) => {
  ...
  if(userKey === masterKey) {
    ...
    res.status(404).json({erorr: `You are not authorised to perform this action.`});
  }
});
app.listen(port, () => {
